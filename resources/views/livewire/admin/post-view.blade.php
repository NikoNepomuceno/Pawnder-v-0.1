    ...
    @if($post)
        <div class="post-card" data-post-id="{{ $post->id }}">
            ...
            <div class="post-details" style="padding: 10px 15px;">
                <span class="post-status {{ $post->status }}">{{ ucfirst(str_replace('_', ' ', $post->status)) }}</span>
        ...
        <div class="lightbox-modal" id="lightboxModal-{{ $post->id }}" tabindex="-1" aria-modal="true" role="dialog"
            style="display:none;">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Get all photo URLs from the post data
                const allPhotoUrls = @json($post->photo_urls);

                // Open lightbox when clicking on an image
                document.querySelectorAll(`[data-post-id='${postId}'] .grid-item`).forEach(function (gridItem) {
                    gridItem.addEventListener('click', function (e) {
                        e.preventDefault();
                        currentImages = allPhotoUrls;
                        currentImageIndex = parseInt(gridItem.dataset.index);
                        showLightbox();
                    });
                });
